# Appendix F: Invariant-Oriented & Relational Data Modeling (v1.5)
...
**Version History**:
- **v1.3**: Initial guidance on entity decomposition, relationships, and views
- **v1.5**: Added cross-references to advanced query, governance, and spatial semantics
...
For complex relationship traversal across entities, v1.5 introduces graph query semantics that complement the invariant-oriented approach:

```yaml
# Example: Find all accounts connected through transactions
graphQuery:
  ...
  traverse:
    ...
    - relationship: credit
      ...
      to: Account
  depth: 1..3
  filter: connection_strength > threshold
```

**Cross-reference**: See Addendum AS (Graph Query Semantics) for complete grammar.

**Use case**: Fraud detection, relationship networks, dependency analysis.

---

### Search Semantics (Addendum AL)

For finding entities based on complex criteria, v1.5 provides search semantics that work across materialized views:

```yaml
# Example: Search transactions by reference or amount
search:
...
**Cross-reference**: See Addendum AL (Search Semantics) for complete grammar.

**Use case**: Transaction lookup, customer search, document retrieval.

---

### Data Governance Semantics (Addendum AX)

Invariant-oriented models must also satisfy compliance requirements. v1.5 provides governance semantics:

```yaml
# Example: Classify transaction data with retention
dataGovernance:
  model: Transaction
  classification:
    level: confidential
...
**Cross-reference**: See Addendum AX (Data Governance Semantics) for complete grammar.

**Use case**: GDPR compliance, SOX requirements, data lifecycle management.

---

### Spatial Type Semantics (Addendum AR)

For location-based invariants (e.g., "transaction must originate from approved region"), v1.5 adds spatial types:

```yaml
# Example: Location-constrained transactions
dataType:
  name: Transaction
  fields:
...
constraint:
  ...
  rules:
    v1: distance(origin_location, account.home_location) < 100km
```

**Cross-reference**: See Addendum AR (Spatial Type Semantics) for complete grammar.

**Use case**: Geo-fencing, fraud detection, compliance zones.

---

### Structured Content Type (Addendum AP)

For entities that contain rich content (e.g., transaction notes with formatting), v1.5 provides structured content:

```yaml
# Example: Rich transaction notes
dataType:
  name: Transaction
  fields:
    notes: structured_content
    
structuredContent:
  format: markdown | html | prosemirror
  sanitization: strict
  max_length: 10000
```

**Cross-reference**: See Addendum AP (Structured Content Type) for complete grammar.

**Use case**: Notes, comments, descriptions with formatting.

---

### Inference-Derived Fields (Addendum AQ)

For fields that are computed via ML models (e.g., fraud risk score), v1.5 supports inference-derived fields:

```yaml
# Example: ML-based fraud score in view
view:
  name: TransactionRisk
  inputs: [Transaction, Account]
  derived:
    fraud_score:
      type: inference
      model: fraud_detection_v3
      inputs: [amount, origin_location, recipient, time]
      refresh: on_read
```

**Cross-reference**: See Addendum AQ (Inference-Derived Fields) for complete grammar.

**Use case**: Risk scoring, recommendations, predictions.
