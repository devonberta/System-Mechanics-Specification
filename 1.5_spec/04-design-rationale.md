# System Mechanics Specification - Design Rationale v1.5
...
**v1.3 Additions**: Entity-Scoped Authority, Authority Mobility, Invariant-Oriented Modeling, Storage Role Separation

**v1.4 Additions**: UI Composition, Experience Router, Field Permissions, Navigation Trees

**v1.5 Additions**: Form Intent Binding, Asset Semantics, Search Capabilities, Scheduled Triggers, Notification Channels, Collaborative Sessions, Durable Workflows, Delegation/Consent, Conversational Experiences, External Integrations, Data Governance, Edge Device Support
...
### Decision: Form Intent Binding (NEW in v1.5)

**What**: Explicit grammar for binding UI forms to input intents with field mappings and lifecycle.

**Why**:
1. **Completeness**: Bridges the gap between UI and intent without prescribing implementation
2. **Validation clarity**: Separates UI-level validation from business validation
3. **User experience**: Enables declarative success/error handling
4. **Transport independence**: Doesn't mandate HTTP, GraphQL, or any specific submission mechanism

**Tradeoff**: More grammar surface area for UI developers to learn

**Alternative Rejected**: Implicit form-to-intent binding
- Would force conventions that vary by framework
- Would make validation unclear
- Would couple UI to transport

**Validation**: Single form grammar works across React, Vue, mobile, and voice interfaces.

---

### Decision: View Materialization Contract (NEW in v1.5)

**What**: Explicit retrieval contracts for how views are keyed, queried, and streamed.

**Why**:
1. **Access patterns**: Makes it clear whether views are entity-scoped, queryable, or singleton
2. **Temporal semantics**: Enables streaming views with windowing and watermarks
3. **Unavailability handling**: Explicit fallback strategies (cached, degraded, wait, fail)
4. **List boundaries**: Prevents unbounded collection fetches

**Tradeoff**: More complexity in view definitions

**Alternative Rejected**: Implicit retrieval patterns
- Would force implementations to guess at intended access patterns
- Would make streaming semantics implicit
- Would prevent optimization

**Validation**: Same view grammar supports KV retrieval, SQL queries, and streaming subscriptions.

---

### Decision: Asset Semantics (NEW in v1.5)

**What**: Binary content (images, documents, videos) as first-class grammar with lifecycle and transformation hints.

**Why**:
1. **Completeness**: Applications need more than just structured data
2. **Lifecycle control**: Assets have retention, expiry, and garbage collection needs
3. **Transformation hints**: Enable optimization without dictating implementation
4. **Access control**: Assets inherit policy from their context

**Tradeoff**: Adds complexity for applications that don't need assets

**Alternative Rejected**: Assets as opaque URL strings
- Would lose lifecycle semantics
- Would prevent optimization hints
- Would make access control implicit

**Validation**: Banking demo supports check images with transformation and retention without custom code.

---

### Decision: Search Semantics (NEW in v1.5)

**What**: Declarative search grammar with ranking, filtering, and highlighting.

**Why**:
1. **User expectation**: Modern applications require search
2. **Transport agnostic**: Doesn't mandate Elasticsearch, Algolia, or any specific engine
3. **Ranking transparency**: Explicit criteria and weights
4. **Context-aware**: Search can be entity-scoped, user-scoped, or global

**Tradeoff**: Complex search requires sophisticated runtime implementation

**Alternative Rejected**: Application-level search implementation
- Would be inconsistent across applications
- Would lose policy integration
- Would duplicate effort

**Validation**: Same search grammar implemented with both embedded full-text and external search service.

---

### Decision: Scheduled Trigger Semantics (NEW in v1.5)

**What**: Cron-like scheduling grammar for recurring and one-time triggers.

**Why**:
1. **Common need**: Many applications require time-based automation
2. **Policy integration**: Scheduled triggers respect same authorization as user-initiated
3. **Declarative**: Schedule expressed as grammar, not code
4. **Execution semantics**: Clear idempotency and retry behavior

**Tradeoff**: Runtime must implement scheduler integration

**Alternative Rejected**: External scheduler with custom integration
- Would bypass policy enforcement
- Would lack observability integration
- Would duplicate trigger logic

**Validation**: Scheduled reports and batch jobs defined in grammar, no custom scheduler code.

---

### Decision: Notification Channel Semantics (NEW in v1.5)

**What**: Multi-channel notification grammar (email, SMS, push, in-app) with templates and delivery tracking.

**Why**:
1. **User experience**: Applications need to notify users
2. **Channel abstraction**: Same notification logic works across channels
3. **Policy integration**: Notifications respect user preferences and opt-out
4. **Observability**: Delivery tracking built-in

**Tradeoff**: Runtime must integrate with notification providers

**Alternative Rejected**: Direct channel-specific APIs
- Would couple application to providers
- Would duplicate policy enforcement
- Would lack consistent tracking

**Validation**: Banking demo sends transaction alerts via email, SMS, and push from single notification grammar.

---

### Decision: Collaborative Session Semantics (NEW in v1.5)

**What**: Real-time multi-user session grammar with presence, locking, and merge strategies.

**Why**:
1. **Modern expectation**: Users expect to collaborate in real-time
2. **Conflict prevention**: Explicit locking and merge strategies
3. **Presence awareness**: Who else is viewing/editing
4. **Session lifecycle**: Clear join/leave/timeout semantics

**Tradeoff**: Complex runtime coordination requirements

**Alternative Rejected**: Application-level WebSocket handling
- Would be inconsistent
- Would bypass policy
- Would lack standard patterns

**Validation**: Banking demo supports collaborative review sessions without custom WebSocket code.

---

### Decision: Durable Workflow Semantics (NEW in v1.5)

**What**: Long-running process grammar with compensation, timeouts, and human-in-the-loop steps.

**Why**:
1. **Complex processes**: Some business processes span days/weeks
2. **Failure recovery**: Explicit compensation and rollback
3. **Human steps**: Support for approvals and manual intervention
4. **State persistence**: Workflow state survives restarts

**Tradeoff**: Requires workflow engine integration

**Alternative Rejected**: Sequential SMS flows only
- Would not survive restarts
- Would lack compensation semantics
- Would not support long delays

**Validation**: Loan approval workflow with multiple human steps and week-long timeouts.

---

### Decision: Delegation and Consent Semantics (NEW in v1.5)

**What**: Grammar for delegating permissions to other subjects with time bounds and scope limits.

**Why**:
1. **Real-world need**: Users need to delegate authority (assistant access, parent/child)
2. **Auditable**: Explicit consent trail
3. **Time-bounded**: Delegations expire automatically
4. **Scope-limited**: Delegate specific permissions, not everything

**Tradeoff**: More complex authorization logic

**Alternative Rejected**: Role-based delegation only
- Would be too coarse-grained
- Would lack time bounds
- Would not capture consent

**Validation**: Banking demo supports temporary account access delegation with audit trail.

---

### Decision: Conversational Experience Semantics (NEW in v1.5)

**What**: Chat/voice interface grammar with intent recognition, context management, and multi-turn flows.

**Why**:
1. **Accessibility**: Voice and chat interfaces broaden accessibility
2. **Channel unification**: Same intent model works for forms, chat, and voice
3. **Context preservation**: Multi-turn conversations maintain state
4. **NLU abstraction**: Doesn't mandate specific NLU engine

**Tradeoff**: Runtime must integrate with conversational AI platforms

**Alternative Rejected**: Forms-only UI
- Would exclude voice interfaces
- Would require separate chat implementation
- Would duplicate intent logic

**Validation**: Banking demo supports both web forms and conversational interface from same intent definitions.

---

### Decision: External Integration Semantics (NEW in v1.5)

**What**: Grammar for third-party service integration with circuit breakers, retries, and fallbacks.

**Why**:
1. **Reality**: Applications must integrate with external systems
2. **Resilience**: Explicit failure handling and circuit breaker patterns
3. **Observability**: Built-in latency and error tracking
4. **Policy integration**: External calls subject to authorization

**Tradeoff**: Runtime must implement integration patterns

**Alternative Rejected**: Direct HTTP clients in work units
- Would lack standard resilience patterns
- Would bypass policy
- Would duplicate error handling

**Validation**: Banking demo integrates with credit bureau API with declarative circuit breaker and retry.

---

### Decision: Data Governance Semantics (NEW in v1.5)

**What**: Compliance grammar with retention policies, data classification, and audit requirements.

**Why**:
1. **Regulatory compliance**: GDPR, CCPA, and industry regulations require governance
2. **Classification**: Explicit data sensitivity levels
3. **Retention**: Automatic deletion/archival based on policy
4. **Right to erasure**: Support for data deletion requests

**Tradeoff**: Additional compliance overhead

**Alternative Rejected**: Manual compliance management
- Would be error-prone
- Would not scale
- Would lack enforcement guarantees

**Validation**: Banking demo enforces 7-year transaction retention and PII classification automatically.

---

### Decision: Edge Device Semantics (NEW in v1.5)

**What**: Grammar for IoT and edge deployment with offline operation, sync strategies, and resource constraints.

**Why**:
1. **Edge computing**: Not all compute happens in cloud
2. **Offline operation**: Edge devices must work without connectivity
3. **Resource constraints**: Memory and bandwidth limits must be respected
4. **Sync semantics**: Explicit merge strategies when reconnecting

**Tradeoff**: Runtime must support offline-first architecture

**Alternative Rejected**: Cloud-only execution
- Would not support offline scenarios
- Would not work for IoT
- Would miss edge optimization opportunities

**Validation**: Banking demo supports ATM (edge device) with offline operation and reconciliation.
...
### Lesson: Complete Applications Need More Than CRUD (NEW in v1.5)

**Insight**: Real applications require assets, search, notifications, scheduling, and collaboration - not just data manipulation.

**Example**: v1.5 adds 28 addendums covering everything from form binding to edge devices.

**Value**: Specification completeness enables building real-world applications without custom extensions.

---

### Lesson: Accessibility is Not Optional (NEW in v1.5)

**Insight**: Universal access requires first-class support, not afterthoughts.

**Example**: Conversational interfaces, accessibility preferences, keyboard navigation - all declarative.

**Value**: Applications are accessible by default, not through retrofitting.

---

### Lesson: External World Integration is Inevitable (NEW in v1.5)

**Insight**: No application is an island - external integrations must be first-class.

**Example**: External integration semantics with circuit breakers, retries, fallbacks.

**Value**: Resilient integration patterns built-in, not ad-hoc.

---

### Lesson: Compliance Cannot Be Bolted On (NEW in v1.5)

**Insight**: Regulatory requirements must be enforced by the platform, not application code.

**Example**: Data governance semantics with classification, retention, and erasure.

**Value**: Compliance guaranteed by runtime, not by developer discipline.
...
8. **Complete over minimal** (v1.5) - Real applications need comprehensive features
9. **Accessible over limited** (v1.5) - Universal access from the start
10. **Compliant over permissive** (v1.5) - Governance built-in, not bolted-on
...
- **Complete** for real-world applications (v1.5)
- **Accessible** to all users (v1.5)
- **Compliant** with regulations (v1.5)

### Evolution from v1.1 to v1.5

**v1.1-v1.2**: Core execution model (flows, data, policies)
**v1.3**: Authority, state machines, invariant enforcement
**v1.4**: UI composition, experience routing, field permissions
**v1.5**: Application completeness - assets, search, notifications, scheduling, collaboration, workflows, delegation, conversational UX, integrations, governance, edge computing

Every constraint in the grammar exists to prevent a specific class of failure. Every flexibility enables a specific class of optimization. Every addition in v1.5 addresses a real-world need encountered when building complete applications. The balance between these creates a system that is both safe and powerful, while being comprehensive enough for production use.
